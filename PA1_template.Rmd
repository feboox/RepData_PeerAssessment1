...
The mean number of steps taken per day is around `r format(mean(stepsPerDay$x, na.rm=T), nsmall=2)`.

The median is `r median(stepsPerDay$x, na.rm=T)`.
...
There are `r sum(is.na(data$steps))` missing values (where the steps values is NA).
...
The mean number of steps taken per day is around `r format(mean(allStepsPerDay$x, na.rm=T), nsmall=2)`.

The median is `r format(median(allStepsPerDay$x, na.rm=T), nsmall=2)`.
